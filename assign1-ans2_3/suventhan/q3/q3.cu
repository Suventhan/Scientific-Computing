...
__global__ void cuda_basic_matrix_multiply(Real* A, Real* B, Real* C) {
...
void sequential_matrix_multiply(Real A[MATRIX_DIM][MATRIX_DIM], Real B[MATRIX_DIM][MATRIX_DIM], Real C[MATRIX_DIM][MATRIX_DIM])	{
	float sum;
...
__global__ void cuda_enhanced_matrix_multiply(Real * A, Real * B, Real * C) {
	float CValue = 0;
...
int main(int argc, char const *argv[])
{
	...
	if (0 == strcmp(argv[1], "-s"))
	{
		...
		sequential_matrix_multiply(A,B,serial_C);
		GET_TIME(t2);
		comp_time = elapsed_time_msec(&t1, &t2, &sec, &nsec);
		printf("N = %d: CPU Time(ms) = %.2f \n", MATRIX_DIM, comp_time);
	}
	//For CPU parallel computation
	else if (0 == strcmp(argv[1], "-p"))
	{
		printf("CPU Parallel Computation\n\n");
		int num_of_threads;
		// check whether # of threads given as arguements
		if(argc < 3){
		...
		GET_TIME(t1);
		float sum;
		//CPU parallel computation part using OpenMP
		#pragma omp parallel for num_threads(num_of_threads) default(none) shared(A,B,C,sum)
		for (int row = 0; row < MATRIX_DIM; row++){
			for (int col = 0; col < MATRIX_DIM; col++){
				sum = 0.f;
				for (int n = 0; n < MATRIX_DIM; n++){
